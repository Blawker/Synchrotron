function insertion_cavity(B,l1,l2,theta,d,color) { // insertion & rejection
  this.B=B;
  this.state=0;
  ...
  this.start_point=function(bloc) {
    ...
    this.a_f=modulo(2*Math.PI+a+this.theta,2*Math.PI);*/
  ...
  this.change_magnetic_field=function(v_p,m,q) {
    if (this.state==1) {
      const r=this.l1/2*Math.tan(Math.abs(Math.PI/2+this.theta/2));
